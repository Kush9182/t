...
function gamestates() {
  ...
  if (gamestate === 'level1') {
    ...
    if(knight.y<500 || knight.isTouching(spikeGroup)){
      knight.x=100;
      knight.y=675;
    }
  ...
  if (gamestate === 'level2') {
    ...
    knightMovement();
    lavaFireballspawn(random(450, 1450), Math.round(random(10,20)));
    ...
    if(knight.isTouching(lavaFireball)||knight.y<225){
      knight.x=200
      knight.y=350
    }
  ...
  if (gamestate === 'level3') {
    ...
    for (var i = 910; i < 1110; i = i + 100) {
      imageMode(CENTER);
      image(downFacingSpikeIMG, i, 450, 100, 100);
    }
...
function lavaFireballspawn(x, f) {
  if (frameCount % f === 0) {
    lavaFireball = createSprite(x, 1000, 50, 100);
...
for(var i=445;i<width-400;i=i+112){  
  ground9 = createSprite(i, 486, 275, 50);
...
for(var i=580;i<height+300;i=i+365){
  ground13 = createSprite(70, i, 350, 600);
...
function invisibleCollidersForLevel3(){
  ...
  collider1=createSprite(400,140,800,10);
  collider2=createSprite(1550,140,800,10);
  collider3=createSprite(width/2,760,1100,10);
  collider4=createSprite(400,420,100,10);
  collider5=createSprite(1550,420,100,10);
  collider6=createSprite(810,380,100,10);
  collider7=createSprite(1110,380,100,10);
  collider8=createSprite(width/2,490,100,10);
  ...
  collider13=createSprite(880,440,10,100);
  collider14=createSprite(1040,440,10,100);
